...
public class MenungguFragment extends Fragment {
    ...
    private DaftarHistoriMenungguAdapter historiAdapter;
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        ApiService.service_get.getHistoryByUser14("Bearer "+token, id_user).enqueue(new Callback<HistoryModel>() {
            @Override
            public void onResponse(Call<HistoryModel> call, Response<HistoryModel> response) {
                if(response.isSuccessful()) {

                    historiAdapter = new DaftarHistoriMenungguAdapter(getActivity(), response.body().getDetailHistoryModels());
